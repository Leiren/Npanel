...
{
    return js_read_clipboard_impl();
}
EM_JS(void, js_write_clipboard_impl, (const char *text), {
    (async(text) => {
        document.getElementById("clipping").focus();
        const rtn = await navigator.clipboard.writeText(UTF8ToString(text));
        document.getElementById("canvas").focus();
    })(text);
})
static void ImGui_ImplGlfw_SetClipboardText(void *user_data, const char *text)
{
    js_write_clipboard_impl(text);
}
#elif
static const char *ImGui_ImplGlfw_GetClipboardText(void *user_data)
{
